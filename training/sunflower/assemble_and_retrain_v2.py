"""
Combines the existing 25 round-1/2 AMED-filtered positives with the new round-3 AMED-filtered
positives, against the SAME 205 existing Indian negatives used for the deployed sunflower-rf-v0
model. SAME 13-feature schema, SAME RandomForestClassifier hyperparameters as
train_rf_experiments_ab.py (Experiment A) -- no methodology changes.

Unlike the prior rounds (5-fold CV only), this adds a genuine held-out test split: fields never
seen during training at all, not just CV-folded. This is an OFFLINE experimental retrain -- it
does NOT overwrite the deployed server/src/services/agricultural/sunflowerRf/model/sunflower_rf_v0.json
artifact. That requires an explicit, separate promotion step.

Run: ../.venv/bin/python3 assemble_and_retrain_v2.py
"""
import json
import pickle
import statistics
...
def load_round3_positives():
    ...
    round3_pool = {f["id"]: f for f in json.load(open("haryana_round3_selected_250.json"))}
...
def main():
    existing_positives = load_existing_positives()
    round3_positives, round3_excluded = load_round3_positives()
    negatives = load_existing_negatives()

    print(f"Existing positives (round 1+2, already in production model): {len(existing_positives)}")
    print(f"Round-3 new positives (AMED-filtered): {len(round3_positives)}")
    print(f"  EXACT_RULE: {sum(1 for p in round3_positives if p['rule_type'] == 'EXACT_RULE')}")
    print(f"  MARGINAL_TIER_B: {sum(1 for p in round3_positives if p['rule_type'] == 'MARGINAL_TIER_B')}")
    print(f"Round-3 excluded (FOUNDER_SIGNAL_AMED_CONFLICT, not used): {len(round3_excluded)}")
    for e in round3_excluded:
        print(f"  {e['field_id']} (Tier {e['tier']}): AMED={e['amed_crop']}@{e['amed_confidence']}")

    all_positives = existing_positives + round3_positives
    ...
    field_ids = [r["field_id"] for r in all_records]
    X = np.array([[r[f] for f in FEATURES] for r in all_records], dtype=float)
    y = np.array([r["label"] for r in all_records])

    # ---- genuine held-out split: fields never touched during training at all ----
    ...
    # 5-fold CV on the TRAIN split only (same hyperparameters as the deployed model)
    ...
    # Fit final model on the TRAIN split, evaluate on the genuinely held-out TEST split
    ...
    with open("experiment_a_v2_rf_model.pkl", "wb") as f:
        pickle.dump({"model": model, "features": FEATURES, "trained_on": "train_split_only"}, f)
    print("\nSaved experiment_a_v2_rf_model.pkl (OFFLINE candidate -- NOT deployed, production sunflower-rf-v0 unchanged)")

    results = {
        "n_existing_positives": len(existing_positives),
        "n_round3_new_positives": len(round3_positives),
        "n_round3_excluded_amed_conflict": len(round3_excluded),
        "round3_excluded_detail": round3_excluded,
    ...
    json.dump(results, open("experiment_a_v2_results.json", "w"), indent=2)
    print("Saved experiment_a_v2_results.json")
